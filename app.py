...
if choice == "Scraper Live":
    ...
    <style>
    .big-font {
        font-size: 60px !important;  /* Augmente cette valeur */
        font-weight: bold;
    }
    </style>
    <p class="big-font">🌍 CoinAfrique Scraper Live</p>
""", unsafe_allow_html=True)
    ...
    with st.expander("Paramètres du scraping", expanded=True):
        ...
        if st.button("Lancer le scraping"):
            ...
            progress_text.text("✅ Scraping terminé !")
...
# ---------------------------
    ...
    if df.empty:
        ...
        for i in range(len(df_top)):
            ...
            with col_img:
                img_url = df_top.loc[i, "image_lien"]
                if pd.notna(img_url) and str(img_url).strip().startswith("http"):
                    try:
                        st.image(str(img_url).strip(), width=150)
                    except Exception:
                        st.write("🖼️ Image indisponible")
