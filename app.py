...
if choice == "Scraper Live":
    ...
    with st.expander("Paramètres du scraping", expanded=True):
        ...
        if st.button("Lancer le scraping"):
            ...
            # Créer des placeholders séparés pour progres et messages finaux
            ...
            for i in range(1, nb_pages + 1):
                ...
                time.sleep(0.5)  # juste pour visualiser la progression

            # Connexion DB et sauvegarde
            ...
            # Affichage final
            status_placeholder.success(f"✅ Scraping terminé : {len(data_all)} éléments ajoutés dans la base {db_choice} !")
            st.dataframe(data_all.head(), use_container_width=True)

            # Nettoyer les placeholders de progression
            progress_text.empty()
            progress_bar.empty()
# ---------------------------
# UPLOAD WEB SCRAPER
